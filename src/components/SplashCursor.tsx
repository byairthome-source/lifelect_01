...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      const isWebGL2 = !!gl;
      if (!isWebGL2) gl = canvas.getContext("webgl", params) as WebGLRenderingContext | null;
      if (!gl) return { gl: null, ext: { formatRGBA: null, formatRG: null, formatR: null, halfFloatTexType: 0, supportLinearFiltering: false } };
    ...
    const blit = (() => {
      ...
      return (target: { fbo: WebGLFramebuffer | null; width: number; height: number } | null, clear = false) => {
        if (target == null) {
    ...
    let curlFBO: ReturnType<typeof createFBO> | null = null;
    ...
    function step(dt: number) {
      if (!velocity || !curlFBO || !divergence || !pressure || !dye) return;
      gl.disable(gl.BLEND);
      curlProgram.bind();
    ...
    function render(target: { fbo: WebGLFramebuffer | null; width: number; height: number } | null) {
      gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
      gl.enable(gl.BLEND);
      drawDisplay(target);
    }

    function drawDisplay(target: { fbo: WebGLFramebuffer | null; width: number; height: number } | null) {
      const width = target == null ? gl.drawingBufferWidth : target.width;
      const height = target == null ? gl.drawingBufferHeight : target.height;
      displayMaterial.bind();
      if (config.SHADING) gl.uniform2f(displayMaterial.uniforms.texelSize, 1.0 / width, 1.0 / height);
      if (dye) gl.uniform1i(displayMaterial.uniforms.uTexture, dye.read.attach(0));
      blit(target);
    }
    ...
    function updatePointerMoveData(pointer: ReturnType<typeof pointerPrototype>, posX: number, posY: number, color: number[]) {
      pointer.prevTexcoordX = pointer.texcoordX; pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width; pointer.texcoordY = 1.0 - posY / canvas.height;
    ...
    function updatePointerUpData(pointer: ReturnType<typeof pointerPrototype>) { pointer.down = false; }
