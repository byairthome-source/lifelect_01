  ...
  for (int i = 0; i < 128; i++) {
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        float flakeSz = max(uFlakeSize, uMinFlakeSize * depth * halfInvResX);
        float dist;
        if (uVariant < 0.5) {
          ...
          float invFlakeSz = 1.0 / flakeSz;
          dist = snowflakeDist(vec2(testX, testY) * invFlakeSz) * flakeSz;
        }
        if (dist < flakeSz) {
          float flakeSizeRatio = uFlakeSize / flakeSz;
