    ...
    <footer className="relative z-10 bg-novu-near-black text-white">
      <div className="container-main py-16 lg:py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-10">
          ...
          <nav aria-label="Company links"><h2 className="text-label-sm text-white-40 uppercase tracking-[0.1em] mb-5">Company</h2><ul className="space-y-3">{[{label:"About Us",href:"/about"},{label:"Become a Dealer",href:"/contact"},{label:"Blog",href:"/blog"}].map((item)=><li key={item.label}><Link href={item.href} className="text-body-sm text-white-40 hover:text-white transition-colors duration-200">{item.label}</Link></li>)}</ul></nav>
